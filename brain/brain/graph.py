...
async def handle_interrupt(msg: dict, broadcast) -> None:
    """Stop that conversation's live stream (checked between deltas, well
    under the contract's 2s). The running run_turn closes the turn with
    `done` and stickies the redirect note for the next user_msg.
    ...
    tasks (Step 7) will pause with their own task_state. Interrupt on an idle
    conversation is a no-op."""
    ctx = _turn_ctx.get(msg["conversation_id"])
    if ctx:
        ctx["stop"].set()
