...
- **3a — Coding-agent orchestration.** The shared managed-command foundation is implemented: structured CLI/script tools, deterministic permission policy, exact approval binding, TaskRuntime admission, bounded redacted logs, Windows descendant containment, and artifact verification. Next, add thin Codex/Claude adapters on that executor ([07-coding-orchestration](systemdesign/07-coding-orchestration.md)). Highest-value, all Lane 1, no GUI brittleness.
