...
- **[DEEPSCAN_AUDIT.md](DEEPSCAN_AUDIT.md)** — 2026-07-28 whole-repo audit (8 parallel scanners: Brain runtime, Brain data/graph, tools/mock/voice/contract, Rust/Tauri+build, UI state/IPC, UI/UX+a11y, third-party API hallucination check, docs-drift+ponytail-ledger). P0-1 through P0-7 and both rule-3 P1 hangs are fixed and gate-verified (see Bugs.md 2026-07-28). **Still open, by tranche (§7 has file:line detail):** P0-8 capsule-overflow at the fixed 360×52 orb size; Tranche 0d UI/UX P1s (composer auto-grow, approval focus-steal, status-strip clipping, 12 a11y blockers); Tranche 1 unbounded growth (`checkpoints.db`, `action` table, `_dirty` map, `digest_cache`); Tranche 2 correctness hardening (`file_read` size guard, atomic writes, doc-parsing caps, `doc_digest` hash-before-extract, broadcast-ordering, embed-lock race, contract/dispatch test gaps); Tranche 3 packaging blockers (compile-time repo path, bundled sidecars, visible console windows, CSP); Tranche 4 doc-only corrections (5 stale status claims, Tier-1-activity doc drift, deprecated `mammoth` call, dead npm deps). Read this file's §7 before starting any of that work — it has the exact fix per item.
