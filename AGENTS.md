...
```powershell
./dev.ps1               # launches Tauri; Tauri owns Brain and Voice
...
No test framework is used anywhere in this repo (plain `asyncio` + `assert` scripts) - do not introduce pytest/jest without a real reason.
