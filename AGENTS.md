...
**Current working situation (2026-08-03).** Phase 0-2 work and the Phase 3 UI foundation are merged to `main`. Always inspect `git status` and the relevant `git diff` before editing and preserve unrelated work. The Phase-3a architectural gate is met; PyInstaller packaging remains scheduled before 3c, and per-client `stream_frame` subscription remains scheduled with 3d.
...
- Phase 3 — not started; no longer gated on Phase 2 closure. The Phase-3a architectural gate is met. Read `systemdesign/07-coding-orchestration.md` and `systemdesign/12-task-runtime.md` before starting. PyInstaller packaging before 3c and per-client `stream_frame` subscription with 3d are just-in-time prerequisites recorded in `phases.md`.
