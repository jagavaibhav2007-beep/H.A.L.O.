...
## A local real-time antivirus can transiently lock freshly linked cargo test executables (Windows) - 2026-07-23
**Symptom:** `cargo test` (and thus `./dev.ps1 -Verify`'s Rust step) fails with `LNK1104: cannot open file '...\target\debug\deps\ui[_lib]-<hash>.exe'` even though nothing was compiled differently and no process holds the file (it doesn't even exist afterwards). **Cause:** on Windows, whichever antivirus is providing active real-time protection on the machine (confirmed here to be a third-party AV rather than Defender, via `Get-CimInstance -Namespace root/SecurityCenter2 -ClassName AntiVirusProduct`) can grab the linker's freshly created exe to scan it before the linker finishes writing it, so the link step fails. This is AV-vendor-agnostic — Defender, Norton, etc. can all do it depending on real-time-scan settings; it's not specific to this project or any one product. Each retry gets further because incremental compilation skips already-linked targets. **Handling:** just re-run `cargo test` — it passes on the next attempt (verified 2026-07-23: fail → fail on a *different* exe → 7/7 pass). If it recurs often on a given machine, that machine's owner can add a real-time-scan exclusion for `ui\src-tauri\target\` in whichever AV is active (a local machine setting, not a repo fix — never assume or require a specific AV product). Don't misread this as a code regression: if the only failing verify step is Rust linking with LNK1104, retry before investigating.
...
## aiosqlite (>=0.21) connections run on a NON-daemon thread — scripts hang on exit — 2026-07-19
Any script/test that finishes with an open `aiosqlite` connection (e.g. LangGraph's `AsyncSqliteSaver` in `brain/brain/graph.py`) prints all-pass and then never exits: aiosqlite parks each connection on a non-daemon worker thread, and the interpreter waits on it forever. There's no public knob; fix at the connection site by daemonizing before the thread starts — `conn = aiosqlite.connect(path); conn._thread.daemon = True; await conn` (private-attr poke, ponytail-commented in `graph.py:_ensure_graph`). Symptom to recognize: a green test run that "hangs after the last print" with zero CPU. Tests that simulate restart must call `graph.aclose()`.
