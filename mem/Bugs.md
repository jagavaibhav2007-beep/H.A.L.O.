...
**Root cause:** `state/store.ts`'s `selectTaskLogs` returned a fresh `[]` literal whenever a task had no logs (see Gotchas.md, "A zustand selector fallback like `?? []` blanks the whole app"). Fresh-reference selector results defeat `useSyncExternalStore`'s identity check and loop React into throwing, which unmounts the entire tree rather than just the task-logs consumer.
**Fix:** replaced the inline `?? []` with a shared module-level `Object.freeze([])` constant (`NO_LOGS`) so the selector returns the same reference across calls when a task has no logs. Verified live: browser-mode workspace went from `innerHTML.length === 0` to a fully rendered 14k-char tree, zero console errors in a fresh tab (the earlier errors were confirmed to be a stale console buffer carried over from a prior failed load, not still firing).
**Never do:** never leave a crash observed live (via mock server, browser preview, or otherwise) as a "logged for later" task when the actual fix is a one-line selector change — investigate it in the same pass once it's been seen failing, rather than deferring solely because it wasn't the item originally being changed.

## Agent recalled 2 of 12 beliefs — a *partially* stale vector index starved the prompt — 2026-08-01
**Severity:** Critical (user-reported). The successor failure mode to the 2026-07-28 "vector search silently returned nothing" entry below — same subsystem, different shape, and the earlier fix's own code comment describes exactly the case it does not handle.
**Symptom:** asked "what do you know about me?", Halo named one thing, while the Memory panel listed a dozen preferences and facts. No error, no log, every gate green.
**Root cause:** the vector index was being treated as the membership list of active beliefs when nothing keeps it complete. Confirmed against this machine's real DB: 12 active beliefs, only **2** indexed; 12 of the 14 indexed rows were dead (archived/superseded). Two independent mechanisms produce that state — (a) `_index_embedding` returns early when `_embed()` yields `None`, so a belief written while the embedder was unavailable is stored but never indexed and *nothing backfills it* (all 10 unindexed rows predate the 2026-07-28 fix; the only 2 indexed ones were written after it), and (b) vec0 consumes its `k`-window *before* `status='active'` post-filters, so pre-fix dead rows still occupy slots. The 2026-07-28 fix only falls through to the recency floor when the vector result is **empty**; here it was non-empty-but-tiny (2 rows), so the fallthrough never fired and 10 facts were silently absent from every prompt. The UI panel disagreed because it reads `live_beliefs(50)` while the prompt read `search_beliefs` — two different queries over the same table.
**Fix:** `memory.retrieve()` now tops the vector hits up with `store.live_beliefs(50)`, deduped by `belief_id`, letting the existing ~1k-token budget trim. Deliberately **not** in `store.search_beliefs`: its other caller (`memory.py:304`, AUDN neighbor lookup) feeds `_decide()`, which can *supersede* beliefs — injecting unrelated recency rows into a nearest-neighbour query risks silent data loss. The top-up is appended **after** the relevance sort, because floor rows have no `distance` and `r.get("distance") or 0.0` would score them as a perfect match and rank them above genuine vector hits. Net effect: the model's view and the panel's view are now the same query. New `check_partial_index_does_not_hide_live_beliefs` in `test_memory.py` (verified failing pre-fix: all 5 unindexed live beliefs missing).
**Never do:** never let a vector/ANN index be the sole gate on what reaches the model — it is an optimization over a set, never the definition of it, and it goes stale through paths that raise no error (an embedder that was offline at write time, a post-filtered `k`-window). Guard the *partial* result, not just the empty one: "non-empty" is not "complete". When a UI surface and a prompt claim to show the same thing, make them share a query, or expect them to drift apart silently.
